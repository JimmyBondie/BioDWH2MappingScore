...
function loadCluster(id) {
  ...
  mappingContent.innerHTML = '<h3>Mapping cluster ' + id + ' (' + node.label + ')</h3>' +
    createTreeTableRecursive(node);
...
function createTreeTableRecursive(node) {
  ...
  for (var i = 0; i < node.ids.length; i++) {
    ...
    if (Object.prototype.hasOwnProperty.call(idPrefixCounts, prefix)) {
      ...
      idPrefixCounts[prefix] = [1, 1 / node.ids.length, Object.values(prefixSourceCounts[prefix]).reduce((partialSum, a) => partialSum + a, 0), 0];
    }
    ...
    text += '<span class="badge rounded-pill ' + color + ' id-pill" style="border-style: solid; border-color: black; border-width: 0px">' + id + '</span>';
  }
  text += '<br/><strong>Names:</strong> ';
  for (var i = 0; i < node.names.length; i++) {
    text += '<span class="badge rounded-pill bg-light text-dark">' + node.names[i] + '</span>';
  }
  var idPrefixes = Object.keys(idPrefixCounts).sort((a, b) => idPrefixCounts[b][3] - idPrefixCounts[a][3]);
  if (idPrefixes.length > 1) {
    ...
    for (var i = 0; i < idPrefixes.length; i++) {
      ...
      text += '<span class="badge rounded-pill ' + color + '">' + prefix + ' (s: ' + prefixScores[3].toFixed(3).replace(/\.0+$/, '') + ')</span>';
    }
  }
  text += '</td>';
  text += '</tr></tbody></table>';
...
function updateMappingInfoText() {
  ...
  var infoText = '<div class="alert alert-success" role="alert">Loaded ' + Object.keys(window.mappingTrees).length + ' mapping clusters</div>';
  infoText += '<table class="table table-bordered"><tbody><tr>';
  for (let i = 0; i < labels.length; i++) {
    var clusterForLabel = Object.keys(window.mappingTrees).filter((id) => window.mappingTrees[id].label === labels[i]).map((id) => [id, window.mappingTrees[id].ids.length]);
    ...
    infoText += topTenForLabel.map((x) => '<li><a href="#mappingContent" onclick="loadCluster(' + x[0] + ')">' + x[0] + ' (' + x[1] + ')</a></li>').join('')
    infoText += '</ul></td>';
  }
...
function setupSearchClusterInput() {
  new autoComplete({
    ...
    data: {
      src: async (q) => {
        return Object.values(window.mappingTrees).filter((v) => {
